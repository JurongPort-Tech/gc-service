...
public class BillAdjParamCOVO extends BillAbstractAdjParamVO {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static final Log log = LogFactory.getLog(BillAdjParamCOVO.class);

	public BillAdjParamCOVO() {}
	...
	public double getTotalAmount(){
		...
		BigDecimal result = rate.multiply(cntr).multiply(other);
		double retVal = result.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();

		if (DEBUG) log.info(result.toString() + " " + retVal);
		return retVal;

		//return (new BigDecimal(""+this.getUnitRate()).multiply(new BigDecimal(""+this.getTotalContainer())).multiply(new BigDecimal(""+this.getTotalOtherUnit()))).doubleValue();
	}
