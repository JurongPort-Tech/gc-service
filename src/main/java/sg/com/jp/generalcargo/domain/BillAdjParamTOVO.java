...
public class BillAdjParamTOVO extends BillAbstractAdjParamVO {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static final Log log = LogFactory.getLog(BillAdjParamTOVO.class);

	public BillAdjParamTOVO() {}
	...
	public double getTotalAmount(){
		...
		BigDecimal result = rate.multiply(time).multiply(other);
		double retVal = result.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
		if (DEBUG) log.info(result.toString() + " " + retVal);

		return retVal;
		//return (new BigDecimal(""+this.getUnitRate()).multiply(new BigDecimal(""+this.getTotalTime())).multiply(new BigDecimal(""+this.getTotalOtherUnit()))).doubleValue();
	}
